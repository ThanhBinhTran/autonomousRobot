...
if __name__ == '__main__':
    ...
    start_node = Node(goal_coordinate)
    goal_node = Node(start_cooridinate)

    obstacles = Obstacles()
    ''' get obstacles data whether from world (if indicated) or map (by default)'''
    obstacles.read(world_name, map_name)

    RRTx = RRTree_x(start_node, step_size, radius, random_area, sample_size)
    RRTx.build(goal_node, plotter, obstacles)
